...
export const POST = withProtection(async (req: NextRequest) => {
  try {
    ...
    const account_id = uuidv4(); // Generate UUID for account_id
    ...
    const [id] = await db("users").insert({
      ...
      account_id: account_id, // Add the generated UUID
    });
...
export const PUT = withProtection(async (req: NextRequest) => {
  try {
    const userData = await req.json();
    const { id, password,  ...updateData } = userData; // Exclude account_id from updates
